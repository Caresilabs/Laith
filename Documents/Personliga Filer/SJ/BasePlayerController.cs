...
public class BasePlayerController :  MonoBehaviour {
	...
	public float runAcceleration = 100.0f;
	...
	void UpdateInput ()
	{
		...
		if (Input.GetKey (KeyCode.D))
			rigidbody.velocity += (Vector3.right * runAcceleration * Time.deltaTime);
		//rigidbody.MovePosition(rigidbody.position + Vector3.right * speed * Time.deltaTime);
		
		if (Input.GetKey(KeyCode.A))
			rigidbody.velocity += (Vector3.left * runAcceleration * Time.deltaTime);
		//rigidbody.MovePosition(rigidbody.position - Vector3.right * speed * Time.deltaTime);
